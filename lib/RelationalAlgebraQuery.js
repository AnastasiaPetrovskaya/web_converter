...
RelationalAlgebraQuery.prototype.convert = function() {
    ...
    // this.alias = this.alias.replace(/\s/g,'')
...
RelationalAlgebraQuery.prototype.union_transformation = function() {
    return new Promise(function(resolve, reject) {
        ...
        if (!re.test(this.query_body)) {
            console.log('err1');
            reject({message:"Запрос операцией объединения не соответствует необходимому шаблону"});
        }
            ...
            .then(function(proection) {
                ...
                if (ctx.right_proection.length != ctx.left_proection.length ||
                    ctx.right_proection.length == 0 || ctx.left_proection.length == 0) {
                        reject({message:'Для каждого операнда теоретико-множественных операций '+
...
RelationalAlgebraQuery.prototype.except_intersect_transformation = function(type) {
    return new Promise(function(resolve, reject) {
        ...
        if (!re.test(this.query_body)) {
            console.log('err1');
            reject({message:"Запрос c операцией вычитания не соответствует необходимому шаблону"});
        }
        var result = re.exec(this.query_body);
...
RelationalAlgebraQuery.prototype.division_transformation = function() {
    return new Promise(function(resolve, reject) {
        ...
        if (!re.test(this.query_body)) {
            console.log('err1');
            reject({message:"Запрос c операцией деления не соответствует необходимому шаблону"});
        }
        var result = re.exec(this.query_body);
        ...
        if (attr_division_r1.length != attr_division_r2.length) {
            reject({message:"Неверно заданы атрибуты таблиц по которым необходимо выполнить операцию деления"});
        }
        ...
        left_operand.make_sql()
            .then(function(res) {
                ...
                where_position = ctx.left_copy.lastIndexOf('WHERE');
                if (where_position == -1) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        this.proection_transformation()
            .then(function(proection_tl) {
                ...
                if (!re.test(this.query_body)) {
                    console.log('err1', this.query_body);
                    reject({message:"После эквивалентных преобразований запрос не соответсвует необходимому шаблону"});
                }
                ...
                var alias_arr = this.alias.split(",");
...
RelationalAlgebraQuery.prototype.multiplication_transformation = function() {
    console.log('mult_transf', this.query_body)
    var old_body = this.query_body;
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (re.test(this.query_body)) {
            ...
            if (close_bracket != -1 && open_bracket != -1) {
                ...
                reject({message: "Не удалось определить скобки, в которые заключена операция реляционной алгебры."});
            }
        } else {
            resolve('');
        }
...
RelationalAlgebraQuery.prototype.delete_extra_brackets = function() {
    var old_body = this.query_body;
    console.log('[' + new Date() + '] delete_extra_brackets : ', old_body);
