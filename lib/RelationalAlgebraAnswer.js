...
AlgebraQueries.prototype.check_answer = function() {
    console.log('this', this);
    var mark = {mark: 5, comment: ""};

    if (!this.student_rows || !this.teacher_rows) {
        mark = {mark: 0, comment: ""};
    } else {
        //TODO перед сравнением стоит отсортировать массивы и объекты массивов

        if (this.student_rows.length != this.teacher_rows.length) {
            mark = {mark: 0, comment: ""};
        } else {
            this.teacher_rows.every(function(row, row_index) {
                console.log('row', JSON.stringify(row));
                console.log('row', JSON.stringify(this.teacher_rows[row_index]));

                if (!this.student_rows.contains(row)) {
                    mark = {mark: 0, comment: ""};
                    return false;
                }
